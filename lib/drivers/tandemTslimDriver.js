...
module.exports = function (config) {
          ...
          if (pkt.valid &&
            ...
            if (receive_seq != pkt.payload['header_log_seq_no']) {
              ...
              if (headerIdFilter.indexOf(pkt.payload.header_id) === -1 || pkt.payload.name  === undefined) {
                console.log('filtered record: ', pkt.payload.header_id + ', ' + pkt.payload.name);
                console.log('filtered: ', pkt);
                end_seq--;
              }
              else{
                clearInterval(sendTimer);
                ...
                console.log('Found newest event:',pkt);
                callback(null,pkt);
              }

              if (receive_seq < end_seq) {
                console.log('We did not find any events');
                clearInterval(listenTimer);
                clearTimeout(abortTimer);
              }
            }
  ...
  var tandemFetch = function (progress, data, callback) {
    ...
    function getNewestEvent(err, result) {
      ...
      console.log('end_seq: ', result.payload.header_log_seq_no);
    ...
    function binarySearch(err, result) {
      ...
      if (err) {
        ...
        if (minIndex <= maxIndex) {
          ...
          console.log('90 day, closest record: ', result);
